...
class TestExpansionValidation:
    ...
    def test_no_database_declared_means_no_read(self, expanded):
        ...
        # OUR check, and only ours. The capture deliberately does not span the
        # `run_checks()` call below: that walks the whole registry, and a query
        # another app's check makes is not something this gate can assert
        # about — it went red on exactly that, on one Python version, for a
        # SELECT this module never issued.
        assert list(captured) == []

        # The whole registry, the way `manage.py check` calls it: the finding
        # must not appear, whatever else the registry does.
